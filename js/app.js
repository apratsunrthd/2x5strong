...
// Round a weight up to the nearest minIncrement
function roundToIncrement(weight, minIncrement) {
  return Math.ceil(weight / minIncrement) * minIncrement;
}
...
async function checkSessionTiming() {
  try {
    ...
    if (sameDay) {
      ...
      showTimingModal({
        title: '📅 BEEN A WHILE',
        body: `It's been ${Math.round(daysSince)} ${dayWord} since your last session. Your body may need time to readjust — consider a deload: drop all weights to 90% for this session to ease back in and avoid injury.`,
...
window.confirmFinish = async function() {
  ...
  try {
    ...
    for (const lr of currentSession.liftResults) {
      ...
      const settings = getGlobalSettings();
      if (allFive) {
        // Round up to nearest minIncrement
        const raw = state.weight + lr.increment;
        newWeight   = roundToIncrement(raw, settings.minIncrement);
        newFailures = 0;
        if (newDeloads > 0) newDeloads--;
      } else {
        newFailures = state.failures + 1;
        if (newFailures >= 3) {
          // Deload: 10% reduction rounded to nearest minIncrement
          newWeight   = roundToIncrement(Math.round(state.weight * 0.9), settings.minIncrement);
          newFailures = 0;
          newDeloads++;
        }
      }
    ...
    <div class="settings-section">
      ...
      <div class="settings-row">
        <div>
          ...
          <div class="text-muted" style="font-size:12px;">Weights round up to this when increasing</div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;">
